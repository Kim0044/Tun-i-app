...
const SignupScreen = ({ navigation }) => {
  ...
  const [isErrorModalVisible, setErrorModalVisible] = useState(false); // Step 1
  ...
  const handleSubmit = () => {
    if (!username || !email || !password || !confirmPassword || !gender || !selectedDate || !dialect) {
 ...
 const randomText = generateRandomString();
        // Get device name and save user data
        DeviceInfo.getDeviceName().then(deviceName => {
          firestore()
            ...
            .set({
              ...
              dialect: dialect,
              password: password, // Consider not storing password directly; better handle via authentication
              score: 50,
              daily: 1,
  ...
  const ErrorModal = () => (
    ...
    >
      <View style={styles.centeredView}>
        ...
        <Pressable style={styles.exitButton} onPress={() => setErrorModalVisible(false)}>
  ...
  return (
    <View style={styles.container}>
      ...
      />
      {!usernameFilled && <Text style={styles.errorText}>This field is required</Text>}
      ...
      <TextInput
        ...
        onChangeText={(text) => {
          setEmail(text.trim()); // Trim whitespace
          setEmailFilled(!!text.trim()); // Update filled state
        }}
      />

      
      {!emailFilled && <Text style={styles.errorText}>This field is required</Text>}
      ...
      {!passwordFilled && <Text style={styles.errorText}>This field is required</Text>}
...
<Text style={[ { fontFamily: 'BauhausStd-Demi' }]} >Dialect</Text>
  ...
  >
    <Text>{dialect ? dialect : "Select Dialect"}</Text>
      ...
      >
        <View style={{ backgroundColor: "white", padding: 22, borderTopLeftRadius: 20, borderTopRightRadius: 20 }}>
          <TouchableOpacity onPress={() => selectGender("Male")} style={styles.genderOption}>
            <Text style={styles.boldText}>Male</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => selectGender("Female")} style={styles.genderOption}>
            <Text style={styles.boldText}>Female</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => selectGender("Others")} style={styles.genderOption}>
...
>
  <View style={{ backgroundColor: "white", padding: 22, borderTopLeftRadius: 20, borderTopRightRadius: 20 }}>
    <TouchableOpacity onPress={() => selectDialect("Bisaya")} style={styles.dialectOption}>
