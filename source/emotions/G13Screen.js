...
const G13Screen = ({ navigation }) => {
  const [displayText, setDisplayText] = useState('');
  const [showTryAgain, setShowTryAgain] = useState(false);
  const [showCorrect, setShowCorrect] = useState(false);
  const [level, setLevel] = useState(41);
...
<Text style={styles.greetingLabel}>Basic:</Text>
    ...
    <TouchableOpacity onPress={() => handleButtonClick('Nasuko')}>
      <Text style={styles.buttonText}>Nasuko</Text>
    </TouchableOpacity>
  </Animated.View>
          <TouchableOpacity
              style={styles.button}
              onPress={() => handleButtonClick('asa ang banyo')}>
              <Text style={styles.buttonText}>Nahadlok </Text>
            </TouchableOpacity>
