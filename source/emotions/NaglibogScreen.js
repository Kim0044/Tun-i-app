...
const handlePress = async () => {
  ...
  useEffect(() => {
    if (showModal && responseText === 'Amazing') {
      ...
      // Reset progress when modal is closed or responseText is not 'Amazing'
      progress.setValue(0);
    }
  ...
  const checkKeyword = async (text) => {
    if (text.toLowerCase().includes('nalipay')) {  // Check if the recognized text includes the correct phrase
      setResponseText('Amazing');
      setShowModal(true);
      setScore(prevScore => prevScore + 1);  // Increment local state score
...
useEffect(() => {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.contentContainer}>
       ...
       {showVideo && (
          <Video
            source={require('../../assets/video/intro-nalip.mp4')}
            style={styles.video}
            resizeMode="cover"
  ...
  />
          ...
          <Text style={styles.greetingText1}>Happiness</Text>
...
>
  ...
  <Video
        source={require('../../assets/audio/Amazing.mp3')} // Adjust the path as necessary
