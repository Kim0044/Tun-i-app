...
const  NasukoScreen = () => {
  ...
  const [level, setLevel] = useState(39);
...
// This effect depends on the responseText state
...
const startCountdown = () => {
  let timer = 0; // Starts from 3 seconds
  const interval = setInterval(() => {
    timer -= 1;
    setCountdown(timer);
    if (timer <= 0) {
      clearInterval(interval);
      navigateToPMA();// Replace this with your navigation function if it's named differently
    }
  }, 1000);
};
  ...
  useEffect(() => {
    if (responseText === 'Great') {
      ...
      Animated.timing(progress, {
        toValue: 100,  // This should match the completion value
        ...
        if (finished) {
          // Delay showing the modal by 6 seconds
          setTimeout(() => {
            setShowModal(true);
...
useEffect(() => {
  ...
  const checkKeyword = async (text) => {
    if (text.toLowerCase().includes('nahadlok')) { 
      ...
      try {
        ...
        if (userDoc.exists) {
          ...
          await firestore().collection('users').doc(currentUserUID).update({
            score: currentScore  // Update the score in Firestore
          });
...
useEffect(() => {
  if (failedAttempts >= 3) {
    ...
    setTimeout(() => {
      ...
      navigation.navigate('Emotions'); // Navigate to "HIA"
    }, 7000);
  }
}, [failedAttempts]);
...
useEffect(() => {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.contentContainer}>
       ...
       {showVideo && (
          <Video
            source={require('../../assets/video/intro-had.mp4')}
            style={styles.video}
            resizeMode="cover"
  ...
  />
          <View style={styles.greetingContainer}>
            ...
            <Text style={styles.greetingText}>Emotions</Text>
          </View>
          <Text style={styles.greetingText1}>Scared</Text>
  ...
  onRequestClose={() => {
    ...
    setCountdown(3);
    setVideoEnded(false);  // Reset to ensure countdown does not show prematurely
  }}
>
  <View style={styles.modalContainer}>
    <View style={styles.modalContent}>
      <Video
        source={require('../../assets/video/GEC.mp4')}
        style={styles.video1}
            resizeMode="cover"
          
        paused={!modalVideoPlaying}
        onEnd={() => {
          setModalVideoPlaying(false);
          setVideoEnded(true);  // Indicate that video has ended
...
>
  ...
  <Video
        source={require('../../assets/audio/Great.mp3')} // Adjust the path as necessary
